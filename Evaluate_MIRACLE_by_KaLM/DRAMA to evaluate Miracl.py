...
def preprocess_dataset(dataset):
    ...
    corpus = {str(item["docid"]): item["title"] + item["text"] for item in corpus_dataset if str(item["docid"])}
...
def batch_encode(model, texts, device, batch_size=16, prompt_name=None):
...
def evaluate_retrieval(dataset, lang):
    ...
    # Generate embeddings
    query_embeddings = model.encode(query_texts, convert_to_numpy=True, device=device, prompt_name="query")
    corpus_embeddings = batch_encode(model, corpus_texts, device, batch_size=16)
...
dataset = load_dataset("miracl/miracl", "yo", trust_remote_code=True)
lang = "Yoruba (yo)"
